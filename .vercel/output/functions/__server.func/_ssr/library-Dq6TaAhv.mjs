import { D as stringifyCsv, b as parseEcfObjects, p as csvLookup, y as parseCsv } from "./button-xNsKyIxq.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/library-Dq6TaAhv.js
...
function collectDialogueKeys(doc) {
...
function objectsFor(catalog, role) {
	...
	const kind = role === "items" ? "item" : role === "blocks" ? "block" : role === "tokens" ? "token" : role === "templates" ? "template" : role === "reputation" ? "reputation" : role === "warfare" ? "element" : role === "galaxy" ? "galaxyconfig" : "faction";
...
function dialogueCsvTable(catalog, doc) {
	const storedText = catalogText(catalog, "dialoguesCsv")?.text;
	const stored = storedText ? parseCsv(storedText) : {
		languages: [],
		rows: {}
	};
...
function writeDialoguesCsv(catalog, doc) {
...
//#endregion
export { writeDialoguesCsv as C, writeDialogues as S, parseDialogueDoc as _, catalogText as a, setGoto as b, dialogueGroup as c, isGotoReset as d, locaLabel as f, objectsFor as g, mergeForeignDialogues as h, blankFunction as i, dialogueStrings as l, looksLikeKey as m, VAR_TYPES as n, dialogueCsvTable as o, localizationTable as p, blankDialogue as r, dialogueDocFor as s, SNIPPETS as t, gotoTarget as u, reindex as v, writeLocalization as w, uniqueDialogueName as x, resolveDialogueText as y };
