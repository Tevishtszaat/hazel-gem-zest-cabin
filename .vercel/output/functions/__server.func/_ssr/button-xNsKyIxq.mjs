...
import { a as beginBusy, i as APP_SUBTITLE, n as APP_NAME, o as endBusy, r as APP_SHORT } from "./router-DINYOVuX.mjs";
import { n as load } from "../_libs/js-yaml.mjs";
import { t as Slot } from "../_libs/radix-ui__react-slot.mjs";
import { n as clsx, t as cva } from "../_libs/class-variance-authority+clsx.mjs";
import { t as twMerge } from "../_libs/tailwind-merge.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/button-xNsKyIxq.js
...
function parseOpen(line) {
	...
	if (!name) name = header.replace(NAME_FIELD, "").replace(ID_FIELD, "").replace(/^[\s,]+/, "").replace(/\s+#.*$/, "").replace(/\}.*$/, "").trim();
	const obj = {
		kind: open[2],
		plus: Boolean(open[1]),
		name,
	...
	if (nameMatch || idMatch) {
		const extra = header.replace(NAME_FIELD, "").replace(ID_FIELD, "").replace(/^[\s,]+/, "").match(/^\s*([A-Za-z][A-Za-z0-9_]*)\s*:\s*(.*)$/);
...
function parseEcfObjects(text) {
	...
	for (const line of lines) {
		...
		if (opener && /^\s*\{/.test(line)) {
			...
			if ((line.match(/\}/g) || []).length >= opens) pushDone(stack.pop());
			continue;
		}
...
function stringifyEcfObjects(objects) {
	const emit = (obj, indent) => {
		...
		for (const [key, value] of Object.entries(obj.fields)) {
			...
			lines.push(`${pad}  ${key}: ${needsQuote(value) || /[:(),]/.test(value) ? `"${value.replace(/"/g, "\\\"")}"` : value}`);
		}
...
var LENGTH_CODES = /* @__PURE__ */ new Set(["hud-wrap"]);
function problemIgnoreKey(problem) {
	return `${problem.code}|${problem.id ?? "root"}|${problem.field ?? ""}|${problem.value ?? ""}`;
}
function visibleProblems(issues, ignored, opts) {
...
var usePdaStore = create()(persist((set, get) => ({
	...
	ignoreProblems: (keys) => set((s) => ({ ignoredProblems: [.../* @__PURE__ */ new Set([...s.ignoredProblems, ...keys.filter(Boolean)])] })),
	unignoreProblems: (keys) => set((s) => {
...
function useProblems(opts) {
	...
	const issues = (0, import_react.useMemo)(() => visibleProblems(raw, ignoredProblems, opts), [
		raw,
		ignoredProblems,
		opts?.includeLength,
		opts?.includeIgnored
	...
	return {
		issues,
		stats: (0, import_react.useMemo)(() => problemStats(issues), [issues]),
		busy,
		raw,
		ignoredCount: (0, import_react.useMemo)(() => raw.filter((issue) => ignoredProblems.includes(problemIgnoreKey(issue))).length, [raw, ignoredProblems])
	};
}
function AppHeader() {
	const project = usePdaStore((s) => s.project);
...
function AppNav() {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("nav", {
		className: "grid grid-cols-3 border-t border-border sm:grid-cols-6",
		children: [
			{
				to: "/",
				label: "PDA"
...
//#endregion
export { suggestionsFor as A, problemStats as C, stringifyCsv as D, splitTokens as E, warmImageCache as F, usePdaStore as M, useProblems as N, stringifyEcfObjects as O, validateProject as P, problemIgnoreKey as S, selectedContext as T, lookupCatalog as _, CHECK_NAME_KINDS as a, parseEcfObjects as b, catalogCounts as c, cn as d, counts as f, iconCandidates as g, getImageUrl as h, CHECKS as i, upsertCsv as j, stripBbcode as k, catalogLoaded as l, decodePdaEscapes as m, BUILTIN_NAMES as n, CHECK_TYPE_KINDS as o, csvLookup as p, Button as r, bbcodeToHtml as s, AppHeader as t, classifyScenarioPath as u, normalizePath as v, resolveIconUrl as w, peekImageUrl as x, parseCsv as y };
